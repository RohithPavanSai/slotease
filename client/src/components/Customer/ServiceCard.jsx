...
export default function ServiceCard({ service, salonId }) {
  ...
  const backendURL = "http://localhost:8080"; // ✅ same pattern as StylistCard

  const handleClick = () => {
    navigate(`/customer/${id}/staff/${salonId}/${service.serviceName}`, { state: { service } });
  };

  // Image handling like StylistCard
  const imageSrc = service.image ? `${backendURL}${service.image}` : "/default-service.png";
